# No crazy war, please — automação de vídeos de fatos

Pipeline em Python que transforma as trends mais quentes do X (Twitter) em um vídeo vertical narrado, em formato explicativo (análise/educacional), pronto para publicar no canal **[@NoCrazyWarPlease](https://www.youtube.com/@NoCrazyWarPlease)**.

**O que é pauta aqui** (ampliado em **2026-08-15**, pedido do usuário; até então era geopolítica e só ela), em cinco famílias:

1. **Geopolítica**, que segue sendo o núcleo — guerra e conflito armado, defesa e armamento, programa nuclear, inteligência e espionagem, diplomacia e tratados, sanções e guerra comercial, disputa por energia e recurso, fronteira e migração, e mudança de poder dentro de um país quando ela muda a posição dele no tabuleiro;
2. **Acontecimento internacional** de grande porte — acidente aéreo ou ferroviário, colapso de estrutura, surto de doença, decisão judicial de repercussão mundial;
3. **Crime** apurado — megaoperação, apreensão, prisão, denúncia, condenação, atentado, crime organizado, tráfico, fraude bilionária;
4. **Alerta e desastre ambiental ou climático** — terremoto, tsunami, erupção, deslizamento, furacão, ciclone, tornado, ventania, temporal, enchente e alagamento, seca, onda de calor, incêndio florestal, e o **alerta oficial** emitido por autoridade;
5. **Risco ambiental de origem humana** — vazamento de óleo ou químico, rompimento de barragem, contaminação, acidente industrial ou nuclear.

**A régua não se abriu junto com a pauta: viés FACTUAL.** O vídeo cobre algo que **aconteceu**, com fonte nominal e número atribuído. Ficam de fora opinião, previsão de terceiros sem órgão oficial por trás, boato, polêmica de rede social, lançamento de produto e resultado de empresa. A previsão de **órgão oficial** é exceção porque é fato: a trajetória que o centro de furacões publicou e o alerta que o serviço geológico emitiu existem, têm autor e têm data. Um fato de tecnologia ou de mercado entra só pela porta do acontecimento: o chip que virou controle de exportação, o cabo submarino cortado, o preço do petróleo que a guerra move.

> **De onde este repo veio.** É o mesmo pipeline do [`automacao-video`](https://github.com/LuSrodri/automacao-video), com outra pauta. Aquele canal cobria tecnologia, IA e mercado; em 2026-07-30 ele **removeu** as contas de OSINT, defesa e geopolítica da coleta e passou a vetar o tema nos prompts. Aqui essas contas são o **núcleo** da curadoria, e a lista foi ampliada de 12 para **200 contas verificadas** (162 de geopolítica até 2026-08-15, quando saíram 14 contas mortas ou protegidas e entraram 52 de desastre, clima, crime e acontecimento internacional). A documentação de mecânica abaixo (edição, auditoria, cartelas, moldura de celular, SEO/GEO) é herdada porque o código é o mesmo — inclusive as datas das decisões que explicam por que ele é assim.

> **Neutralidade — a regra que este canal tem e o outro não.** O vídeo explica a disputa e **não escolhe lado nela**. Na prática: toda afirmação disputada é **atribuída** a quem a fez ("o governo ucraniano afirma", "o Ministério da Defesa russo diz"); número de **mortos e feridos** só entra com a fonte colada nele; quando os dois lados dão números diferentes, o vídeo diz os dois e diz que divergem; e adjetivo de julgamento sobre país, governo, exército, povo ou religião é proibido em título, descrição e narração. A regra está em **três lugares de propósito** — a curadoria das trends (`x_client.py`), a seleção e o roteiro (`escritor.py`) e a auditoria pró-leigo (que reprova e manda reescrever) —, porque é a única regra cuja violação não estraga um vídeo: estraga o canal. Ver "Como funciona a neutralidade".

> **Idioma — o canal decide, nunca o modelo.** Este canal publica **tudo** em português (título, descrição, narração e **capa**). O código continua bilíngue (a flag `-usa` produz tudo em inglês, para um eventual canal irmão), e o idioma é dado do pipeline (`cfg.publico`), não coisa a deduzir do conteúdo: os prompts do roteiro (`FOCO_BRASIL`/`FOCO_USA`) e o da capa (`pipeline/thumbnail.py`) recebem a regra explícita, e a capa ainda é **conferida em código** depois da resposta, com uma segunda chamada cobrando a correção quando sai fora.

1. **Coleta** os posts das últimas 24h da **lista fixa de 200 contas** do canal (`CONTAS_PADRAO` em `pipeline/config.py`; `X_ACCOUNTS` no `.env` a substitui) — **OSINT e monitoramento de conflito primeiro**, porque são elas que publicam clipe de vídeo em volume, e depois governos e forças armadas, organismos multilaterais, think tanks, imprensa internacional, **monitores de sismo e de tempo extremo, agências de alerta (serviço geológico, centro de furacões, defesa civil), polícias e Ministério Público** e Brasil; todos os handles foram verificados um a um contra `/2/users/by` da X API — via X API oficial v2, pay-per-use, com teto de leitura configurável, por **dois caminhos complementares**: a **busca por relevância** (`/2/tweets/search/recent`, mais a varredura opcional `has:videos`) e a **timeline cronológica** das contas (`/2/users/:id/tweets`) — ver "Como funciona a coleta por timeline". O **GPT** então os sumariza nas **10 trends mais quentes**, ordenadas pelo **valor da informação** (vazamento, documento, exclusivo, urgência, número inédito) **antes** do engajamento, cada uma com resumo, `valor_informativo`, `urgencia`, engajamento, nota de apelo visual e **quantos posts têm clipe de vídeo nativo** (a mesma chamada da coleta já traz o tipo de mídia de cada post). O GPT devolve o **inventário completo** dos posts com vídeo de cada trend (`posts_video`) à parte da lista de posts mais centrais (`posts`, truncada): a contagem sai da união dos dois, senão uma pauta que **tem** clipe — só não entre os posts mais centrais — seria vetada como se não tivesse material. Os posts com vídeo vão para a frente da lista, que é onde o lookup de mídias corta.
2. **GPT 5.6 Luna** classifica cada candidata (**macrotema** + **imagem mental**) — sem filtro nem score: todas as candidatas seguem vivas para a seleção. Os macrotemas cobrem as cinco famílias da pauta, com a geopolítica ocupando a maior parte da lista porque é o núcleo do canal (`guerra-conflito`, `defesa-armamento`, `nuclear-estrategico`, `inteligencia-espionagem`, `diplomacia-tratados`, `sancoes-comercio`, `energia-recursos`, `poder-interno`, `fronteiras-migracao`, `americas`, `crime-seguranca`, `desastre-natural`, `clima-extremo`, `risco-ambiental`, `internacional-geral` e `outro`). A granularidade é maior de propósito: com dois ou três rótulos, um ciclo quente de conflito ocuparia o canal inteiro e o **rodízio de temas dos Shorts** (item 3) não teria para onde ir — e é pelo mesmo motivo que o desastre entra em **três** rótulos (o geológico, o meteorológico e o ambiental de origem humana) em vez de um "desastres" só: uma temporada de furacões tem fato novo todo dia e sozinha tomaria o canal.
3. **GPT 5.6 Luna** escolhe a trend guiado **somente pela audiência**: recebe os **últimos 100 vídeos publicados no canal selecionado com as métricas reais** (views/likes em tempo real, YouTube Data API) e os **campeões de retenção** (YouTube Analytics), e escolhe a candidata com a maior chance de performar com esse público — repetir o tipo de conteúdo que está performando é bem-vindo, **sem cota de variedade**. Nos **Shorts** vale, desde 2026-08-04, um **rodízio de temas** aplicado em código antes da escolha: as candidatas do macrotema do Short anterior saem da disputa, de modo que **cada Short sai de um tema diferente do anterior** (o veto cede se zerar as candidatas — melhor repetir o tema do que não publicar). O formato longo não tem rodízio. Logo depois do rodízio entra, desde 2026-08-15, a **prioridade por engajamento**: os macrotemas cujo *viewed vs swiped away* está **acima de 56%** ficam com a disputa só para eles — ver "Como funciona a prioridade por engajamento". As métricas chegam ao prompt **normalizadas pela idade** (**views por hora** ao lado das views acumuladas): views acumuladas medem há quanto tempo o vídeo está no ar tanto quanto medem qualidade, então o pico de um ciclo já encerrado continuaria sendo o maior número da lista por dias depois do assunto morrer. É o views/h que mostra o ciclo esfriando — vídeos recentes de um macrotema rendendo bem menos por hora que os antigos do mesmo macrotema — e faz o modelo trocar de assunto sozinho. Regras duras, aplicadas em código: **candidata sem nenhum post com clipe de vídeo sai da disputa** (o formato é montado só com clipes do X) e a **verificação anti-repetição** — o GPT confere se a escolhida cobriria o **mesmo fato** de um vídeo publicado nas últimas 36h sem desenvolvimento novo; se sim, ela sai da disputa e a seleção refaz (se todas as candidatas caírem em uma das regras, não há vídeo).
...
```powershell
.\.venv\Scripts\Activate.ps1
python main.py                    # Short vertical (o formato do canal)
python main.py --long-take        # vídeo longo de análise (16:9)
python main.py -usa               # idem, tudo em inglês (canal irmão; não existe hoje)
python main.py --long-take -usa
```

Autorização do canal (uma vez só, não gera vídeo):

```powershell
python main.py --auth-youtube
```

Ele abre o navegador; **escolha a conta de marca do canal `No crazy war, please`** na tela de consentimento e o refresh token é gravado em `YOUTUBE_REFRESH_TOKEN` no `.env`. O client OAuth é o mesmo dos outros canais do usuário, então selecionar o canal errado ali publica geopolítica no canal errado. (`--auth-youtube-usa` existe para o canal irmão em inglês, que hoje não existe.)
...
mesmo pipeline — mesma coleta do X, mesmo
crédito de reprodução no canto superior direito — com outra direção editorial:

- **Enquadramento**: explica um acontecimento cobrindo **de 3 a 5
  tópicos** — recortes diferentes do mesmo fato, costurados por causa e efeito,
  nunca como lista de bullets falados. As quatro óticas do canal são a fonte
  natural desses tópicos: o **TERRENO** (quem fez o quê, onde, com que
  capacidade), os **INTERESSES** (quem ganha, quem perde, que aliança é
  testada, quem banca quem), o **DINHEIRO** (energia, rota comercial, sanção,
  contrato, preço) e o **PRECEDENTE** (o que este fato autoriza a acontecer
  depois). Elas **não são uma cota**: quando o fato não tem leitura econômica
  real, o roteiro cobre outro recorte (direito internacional, política interna
  de quem decidiu, reação do vizinho) em vez de inventar uma — forçar a ótica
  ausente produz exatamente a frase de analista vazia que a auditoria reprova.
- **Espectador**: o adulto que acompanha o noticiário internacional pela
  manchete e chegou com a pergunta que a manchete não respondeu — **por que**
  isso está acontecendo, e o que muda por causa disso. O payload obrigatório é
  o que o acontecimento muda **no tabuleiro** (quem ganha posição, que rota ou
  fronteira muda de status, que preço ou fornecimento é afetado, em que prazo).
  Futurologia ("isso pode virar uma guerra mundial") e conselho ao espectador
  reprovam na auditoria.
...
- **Capa customizada** (`pipeline/thumbnail.py`): um quadro real do vídeo (2s,
  já com o fundo e o celular) escurecido, com 2 a 5 palavras em Archivo Black na
...
- **Celular sobre a superfície** (`pipeline/cenario.py`): a orientação do aparelho
...
| --- | --- | --- |
| `X_ACCOUNTS` | vazio | Opcional: usa somente estas contas no lugar da lista fixa de 200 contas do canal (`CONTAS_PADRAO`, `pipeline/config.py`) |
...
| `JANELA_HORAS` | `24` | Idade máxima dos posts coletados. Alargar **não** custa mais na X API (o teto é o `X_MAX_POSTS`; a janela só decide de que intervalo saem esses posts). Case com a **cadência do cron**, não com o formato: em produção este canal roda com `6` (4 execuções por dia, desde 2026-08-15 — era `8` para 3 execuções) — ver a nota abaixo |
...
| `MAX_CARTELAS` | `1` | Cartelas de imagem nos momentos-chave, que tomam a tela do celular pelo deslize do carrossel (`0` desliga). Caiu de 2 para 1 com o Short de 25s: cada imagem tira ~4s de clipe da tela |
...
| `YOUTUBE_CATEGORY_ID` | `25` | Categoria do YouTube (25 = News & Politics, a do canal) |
...
> **Ative também a YouTube Analytics API** no mesmo projeto do Google Cloud, e reautorize se o refresh token for anterior ao escopo `yt-analytics.readonly`. É de lá que vêm os campeões de retenção e o `engagedViews` que sustenta a prioridade por engajamento — sem ela a execução **aborta** (fail-fast), com o link da ativação na mensagem.

**Cadência em produção:** o cron job `nocrazywarplease` no Render roda `30 10,14,18,22 * * *` (UTC) — **4 Shorts por dia**, às 07:30, 11:30, 15:30 e 19:30 (BRT), todos dentro do dia brasileiro. Subiu de 3 para 4 em **2026-08-15**, e `JANELA_HORAS` desceu de `8` para `6` na mesma mudança, para a janela de coleta continuar casada com o intervalo entre execuções. Os minutos são `:30` de propósito, para não coincidir com os crons dos outros canais do mesmo projeto do Google Cloud — **a cota de upload da Data API é do projeto, não do canal** (1.600 unidades por upload + 50 pelo comentário de abertura), então subir a frequência aqui come cota dos outros.

### O canal irmão em inglês (não existe hoje)

O código carrega a máquina de dois canais herdada do repo de origem: cada canal tem seu próprio refresh token e a seleção é automática pela flag `-usa` (`YOUTUBE_REFRESH_TOKEN` para o português, `YOUTUBE_REFRESH_TOKEN_USA` para o inglês, autorizado com `--auth-youtube-usa`). Hoje só o canal português existe e só ele está no cron — a variável `..._USA` fica vazia. Ficou de pé porque não custa nada em runtime e porque é a diferença entre abrir um canal irmão em inglês num comando e refazer toda a camada de idioma.

O pipeline é **fail-fast**: credenciais ausentes/quebradas, falha ao ler os últimos vídeos ou os campeões de retenção, classificação indisponível, verificação de vídeo repetido indisponível e falha no upload — tudo isso derruba a execução com erro explícito (para o agendador poder alertar), em vez de seguir e degradar o vídeo em silêncio. As leituras do canal acontecem logo no início, antes de qualquer chamada paga (X/OpenAI). Já os clipes são obrigatórios: se nenhum clipe dos posts da trend puder ser baixado, a execução aborta (o formato não admite imagem estática). Se o upload falhar, o vídeo continua salvo em `output/` e registrado em `videos.txt` para publicação manual.

## Publicação automática no TikTok (via Zernio) — **desligada neste canal**

> **Neste repo `TIKTOK_PUBLICAR=0`, e é assim que deve ficar até existir uma conta de TikTok da marca.** O código veio inteiro do repo de origem e funciona, mas a conta que o Zernio tem conectada é a **pessoal do usuário (@lusrodri)**, que é o TikTok do *outro* canal: ligar a variável aqui despejaria os vídeos de geopolítica no perfil errado. Para ligar um dia, conecte a conta da marca no Zernio, preencha `ZERNIO_ACCOUNT_ID` (obrigatório quando há mais de uma conta) e só então `TIKTOK_PUBLICAR=1`. O resto desta seção descreve como a integração funciona.

O **mesmo arquivo** que vai para o YouTube é publicado no TikTok na mesma execução. Nada é gerado de novo: sem coleta extra, sem roteiro extra, sem narração extra — o **custo adicional é zero**. Vale só para o **canal em português** (`python main.py`, sem `-usa`): idioma é regra de canal, então numa execução em inglês a publicação não acontece nem se a variável estiver ligada.
...
3. Ligue com `TIKTOK_PUBLICAR=1` — no `.env` local e, em produção, na env var do cron job. O formato que o TikTok premia é o Short 9:16; o `--long-take` é 16:9 e cabe mal no feed.
...
Fontes (formato longo)      <- posts do X que originaram a trend
...
O laço fecha **antes do TTS** de propósito. Cada tentativa extra custa roteiro + visão, e **nenhuma custa narração** — que é o crédito caro. Por isso o **piso de duração continua abortando seco**: narração curta é defeito do roteiro, e trocar de tema não conserta isso, só paga o ElevenLabs de novo. Se as 3 candidatas falharem, aí sim a execução aborta, e as alavancas são as de sempre: alargar `JANELA_HORAS`, subir `X_MAX_POSTS` ou revisar as contas acompanhadas.
...
- **De onde vêm** — exclusivamente das **fotos dos posts da trend** (que o pipeline já lia da X API e descartava no filtro de tipo: são o material mais barato, vêm no mesmo lookup e estão no assunto por construção). Nenhuma chamada nova de API, e nada de busca de imagem em banco. A **og:image das notícias** completava o pool até **2026-08-16**, quando saiu junto com a busca do Firecrawl.
- **Como aparecem** — renderizadas no **tamanho exato da tela** do aparelho: a imagem entra inteira (nada de recorte que corte rosto ou número) sobre um fundo feito dela mesma, ampliado, borrado e escurecido — o mesmo tratamento que o clipe já recebe —, com o **crédito próprio numa faixa na base** (`Reprodução: X / @conta` ou `Reprodução: reuters.com`; `Image Credit` no `-usa`). O movimento é o **deslize** (ver "Como funciona a moldura de celular"); este módulo só desenha o quadro parado. Substituiu em 2026-08-09 o cartão branco com sombra que subia de baixo do quadro — com a imagem ocupando a tela inteira, o problema de "cartão pequeno perde a disputa pela atenção", que em 2026-08-04 tinha sido tratado aumentando o cartão, deixou de existir.
...
Pedido do usuário em **2026-08-09**: *"troque o formato para uma moldura de um smartphone sobre uma cama; o celular ficará em pé ou deitado a depender da orientação do vídeo; uma mão surgirá, arrastando o vídeo para esquerda, para aparecer as imagens, e depois arrastando a imagem para direita, para voltar o vídeo"*. Substituiu a **sala de estar com TV**, que só o `--long-take` usava — agora a moldura vale para os **dois formatos**. Em **2026-08-10** o mesmo pedido foi revisado: a cama passou a ser uma **foto** (`fundo-cama.png`), a moldura do aparelho foi refeita e a **mão saiu** — o deslize do carrossel ficou. Em **2026-08-14** o usuário trocou a foto de novo, agora por uma **mesa de madeira**: o arquivo passou a se chamar **`fundo.png`**, sem descrever o conteúdo, porque quem escolhe a superfície é o usuário e o nome anterior já estava mentindo sobre o que a imagem mostrava. Em **2026-08-15** veio a troca seguinte, por uma superfície de **cimento queimado claro** — e foi ela que mostrou que o tratamento ainda tinha um número calibrado na foto antiga (ver abaixo).

**O cenário** (`pipeline/cenario.py`) é a foto **`fundo.png`** da raiz do projeto com o celular desenhado em Pillow por cima. A saída é um PNG **opaco em tudo menos no retângulo da tela**: a montagem põe o conteúdo atrás e sobrepõe o PNG, então o corpo do aparelho **recorta** tudo sem precisar de máscara no ffmpeg — é esse recorte que faz o carrossel funcionar.

**O fundo** entra por **escala de cobertura + corte central** (o equivalente Pillow do `force_original_aspect_ratio=increase` + `crop`, para a textura não esticar), com desfoque leve, dessaturação, escurecimento e vinheta: a foto é fundo, e sem esse tratamento a faixa que sobra em volta do aparelho disputa atenção com a tela. Trocou (**2026-08-10**, pedido do usuário) a cama que o Pillow desenhava — colcha em gradiente de linho, travesseiro, vira do lençol e dobras desfocadas —, que saiu inteira. O tratamento **não presume nada sobre a foto** — proporção, cor ou assunto —, que é justamente o que permitiu trocar a cama por madeira em 2026-08-14 sem tocar em código de render. O arquivo é **obrigatório**: sem ele a montagem aborta, em vez de sair com um fundo que o usuário não escolheu.

**O escurecimento é MEDIDO, não fixo** (2026-08-15). Até a foto de cimento, o brilho era um fator único de `0,82` calibrado na madeira. A madeira tem luminância média **42** (de 255) e saía em **34**; o cimento tem **170** e sairia em **139** com o mesmo fator — mais claro que boa parte dos clipes, com o quadro puxando o olho para a moldura em vez de para a tela, e a legenda branca do Short (que cai sobre o fundo quando o aparelho está deitado) disputando com a superfície. Agora `cenario.py` mede a foto e usa `min(0,82; 96 / média)`, com piso em `0,35`: foto escura sai exatamente como saía antes, foto clara é puxada para a média-alvo **96** — onde o cimento ainda lê como cimento (a textura e as manchas sobrevivem) e já está abaixo do miolo dos clipes. O fator do cimento ficou em **0,566**. É a mesma ideia do resto do módulo: o que precisa continuar valendo quando a foto troca é a **hierarquia** (a tela é o ponto mais claro do quadro), não um número calibrado numa textura que já saiu.

**O aparelho** é montado em duas peças concêntricas com a tela, cada uma somando a própria espessura ao raio de dentro (curvas paralelas no canto — raio repetido em espessuras diferentes é o defeito clássico de moldura desenhada): um **trilho de alumínio** externo, com gradiente vertical e dois reflexos — o especular forte na aresta de cima, de onde vem a luz, e um fraco na de baixo, que é a luz da superfície voltando no metal —, e o **bezel preto** fino por dentro dele. Completam a peça os **botões** de power e volume (vinco escuro mais face clara, salientes no perfil do trilho — uma cor só sumia no meio do gradiente), a **sombra em duas camadas** (a de contato, curta e escura, que gruda o aparelho na superfície; a ambiente, larga e difusa, que dá o peso), a **sombra interna** na borda da tela, por onde o conteúdo do vídeo passa ganhando profundidade de vidro, e um **reflexo** diagonal de alfa baixo. Substituiu (**2026-08-10**) a moldura de borda única com um fio de luz correndo por dentro dela, que lia como adesivo: celular nenhum tem esse contorno.
...
**A legenda do Short** continua dentro da tela quando o aparelho está em pé. Com ele **deitado**, a tela tem ~440px de altura e a palavra cobriria o clipe inteiro, então ela desce para o **fundo, abaixo do aparelho** (`cenario.area_legenda`) — sobra espaço de sobra, já que o celular deitado ocupa menos de um terço da altura. Os últimos **14%** do quadro ficam reservados: é onde o Shorts e o TikTok desenham título, @ e botões.
...
**A mão saiu** (**2026-08-10**, pedido do usuário: *"remova a mão e o toque, mas pode manter a animação de deslizar"*). Era uma silhueta desenhada em Pillow — punho fechado, indicador estendido, halo de toque na ponta — que subia de fora do quadro antes de cada arrasto, tinha o `x` comandado pelo mesmo `s(t)` do carrossel e descia depois. Foram embora a silhueta (`gerar_mao`), o halo de toque, a segunda rampa de presença e a entrada de PNG dela no ffmpeg; **o carrossel continua igual** — o conteúdo desliza sozinho. Não reintroduzir sem pedido explícito.
...
**Janela mínima.** Uma imagem precisa de `MIN_JANELA_CARROSSEL` (1,84s = os dois deslizes de 0,42s mais 1,0s de leitura com a imagem parada); abaixo disso ela entraria e já sairia, sem ninguém ler o que mostra. O valor não mudou com a saída da mão — antes o 1,84s era `2 × (arrasto + entrada + saída da mão)`. `DUR_MINIMA` das cartelas (2,2s) e das figuras (2,6s) já fica acima, e a montagem descarta com aviso o que chegar menor.
...
É a única regra deste canal que não existia no repo de origem, e a única cuja violação não estraga **um** vídeo — estraga o canal. Um canal automatizado de geopolítica publica, sem revisão humana, texto escrito a partir de posts de contas que **são parte** da disputa que ele cobre: as contas de OSINT, os ministérios da defesa, as chancelarias. Sem uma regra dura, o roteiro adota a voz da fonte mais barulhenta do dia.

A regra, na prática, é **atribuir**:

- Afirmação disputada nunca é dada como fato — vira "o governo ucraniano afirma", "o Ministério da Defesa russo diz", "a Reuters confirmou com duas fontes".
- **Número de mortos e feridos só entra com a fonte colada nele.** É o dado mais disputado de qualquer conflito; sem atribuição, não entra.
- Quando os dois lados dão números diferentes para a mesma coisa, o vídeo **diz os dois e diz que divergem**. Essa divergência costuma ser o fato mais interessante do vídeo, não um problema a resolver.
- Adjetivo de julgamento sobre país, governo, exército, povo ou religião é proibido em título, descrição e narração ("regime sanguinário", "agressão covarde", "heroica resistência"). Também: torcida, deboche e celebração de baixa.
- Previsão inventada é proibida ("isso pode ser o começo da Terceira Guerra Mundial") a menos que esteja no material recebido como cenário atribuído a alguém. **Previsão de órgão oficial citada com o nome dele não é previsão inventada** — a trajetória do furacão, o alerta de tsunami e o aviso de temporal são fatos datados, com autor.
- A **disputa** que fecha o Short (a alavanca de comentário) é sobre o fato e sobre a conta — quem pagou, o que custou, o que se ganhou —, **nunca** sobre quem é o vilão. Se a única forma de gerar disputa fosse acusar um lado, o roteiro fecha sem disputa.

Com a pauta ampliada em **2026-08-15**, a mesma regra ganhou **duas formas próprias**, nos mesmos três lugares (curadoria, roteiro e auditoria):

- **Crime — presunção de inocência.** Quem foi preso, indiciado ou apontado é **suspeito** ou **acusado** até a condenação, e toda acusação vem atribuída a quem a fez (polícia, Ministério Público, tribunal). Proibido tratar acusado como culpado, expor pessoa comum não condenada (nome completo, foto, endereço) e descrever violência em detalhe gráfico. O vídeo cobre a operação, o número apreendido, o mecanismo do esquema e a decisão da autoridade — não o espetáculo do crime.
- **Desastre — balanço atribuído e marcado como parcial.** Mortos, feridos, desaparecidos e desabrigados só entram com a fonte colada, do mesmo jeito que as baixas de um conflito; balanço de rede social apresentado como oficial e pânico no lugar do dado ("o mundo nunca viu nada assim") são proibidos; e enquanto o número oficial estiver mudando, o vídeo diz que é parcial e de quem ele é.

Ela é repetida em **três camadas** de propósito, seguindo a mesma lição que criou a auditoria pró-leigo no repo de origem — *regra de comportamento nunca fica só no prompt do roteirista*:

| Camada | Onde | O que faz |
| --- | --- | --- |
| Curadoria | `x_client.py` (`INSTRUCOES_RESUMO`) | o resumo da trend já registra quem afirmou o quê, e mantém as duas versões quando elas divergem |
| Seleção e roteiro | `escritor.py` (`INSTRUCOES_SELECAO`, `INSTRUCOES_ROTEIRO`, `INSTRUCOES_ROTEIRO_LONGO`) | descarta a candidata que só existe como acusação sem nada verificável, e escreve atribuindo |
| Auditoria | `escritor.py` (`INSTRUCOES_AUDITORIA_LEIGO`, `..._LONGO`) | **reprova e cobra uma reescrita** quando o texto publicável viola qualquer um dos itens acima |

A auditoria é a camada que vale: ela lê o título, a descrição e a narração **prontos** e é a última coisa entre o texto e o upload.

## Como funciona a prioridade por engajamento (swipe-away)

Desde **2026-08-15**, a pauta é escolhida **primeiro** pelo tema que o feed do Shorts engole, e só depois pelo resto.

**A métrica.** A YouTube Analytics API **não** tem uma métrica chamada "swipe away". O que ela tem é `engagedViews` — "o número de vezes que os vídeos do canal foram vistos **além dos segundos iniciais**" —, que é exatamente o numerador do painel "Assistiram × Passaram direto" do Studio. Os dois lados saem de uma conta só:
...
O **piso do canal é 56% de engajamento** (`PISO_ENGAJAMENTO`, `pipeline/youtube.py`), ou seja, no máximo **44% de swipe-away**.

**Por tema, não por vídeo.** O engajamento é medido por vídeo, mas a régua é por **assunto**: a pergunta não é "este vídeo segurou o feed", é "o feed engole este tema". Então `anexar_engajamento` cola a taxa em cada um dos 100 últimos publicados e `_engajamento_por_macrotema` **soma os números crus** (`engagedViews` e `views`) dentro de cada macrotema, dividindo uma vez só no fim. Somar os brutos, e não tirar média das taxas, é o ponto: **média de médias** deixaria um vídeo de 30 views pesar igual a um de 30 mil, que é exatamente o erro que o piso existe para evitar.

Um macrotema só vale de régua acima de **`PISO_VIEWS_MACROTEMA` = 100 views somadas**. Sem esse segundo piso, um tema com um único vídeo de 12 views e 75% de engajamento (nove pessoas) passaria a mandar na pauta. Tema sem base **não entra na prioridade nem é penalizado por ela**: "não sabemos" não é "vai mal".

**O portão é código, não pedido de prompt.** Como o rodízio, ele roda em `selecionar_trend` antes da chamada ao modelo — instrução de prompt sobre número compete com todas as outras instruções do prompt, e a que o dono do canal fixou não é para competir. A ordem das duas regras importa e é deliberada: **o rodízio corta primeiro**, a prioridade escolhe o melhor tema **entre os que sobraram**. Assim dois Shorts seguidos do mesmo assunto continuam proibidos, que é a regra mais antiga e mais estreita.

O portão **cede** em dois casos, sempre com log: quando nenhuma candidata é de tema acima do piso, e quando nenhum tema tem base medida (canal novo, vídeos de poucas horas — a Analytics atrasa 2-3 dias). Trocar um vídeo bom por nenhum vídeo é um preço que uma preferência não paga. O único veto absoluto continua sendo o de **vídeo repetido**.

O bloco `ENGAJAMENTO POR TEMA` vai junto para o prompt de seleção mesmo quando o portão cede — o modelo precisa saber **por que** aquelas candidatas estão na mesa —, e o swipe-away dos campeões chega também ao **roteirista** (`_resumo_estilo`), porque quem decide esse número é a pergunta de abertura e o primeiro clipe.

**Custo:** zero chamadas a mais. `top_retencao` e `anexar_engajamento` precisam da mesma consulta, e `_linhas_analytics` a serve das duas com um cache de processo.

> **Como os temas novos entram na conta.** Os cinco macrotemas criados em 2026-08-15 (`crime-seguranca`, `desastre-natural`, `clima-extremo`, `risco-ambiental`, `internacional-geral`) nascem **sem base medida** — nenhum vídeo publicado, nenhuma view somada —, então o portão não os prioriza nem os pune. Quem abre a porta para eles é o **rodízio**, que roda **antes**: quando o Short anterior foi do tema campeão, as candidatas dele saem da disputa, o portão não encontra nenhuma candidata prioritária e **cede** — e é aí que um tema novo é escolhido, mede o próprio engajamento e passa a disputar de igual para igual nas execuções seguintes. Com 4 Shorts por dia isso acontece já no segundo do dia; não é preciso mexer no piso de 56% para a pauta nova aparecer.

## Como funciona a estrutura em cinco blocos

Todo roteiro — Short e `--long-take` — segue a mesma ordem de aula bem dada:

1. **PERGUNTA ESQUISITA** (a primeira frase, campo `pergunta`): concreta, estranha e específica, com coisa/número/gente dentro ("quanto custa afundar um navio de 500 milhões de dólares com um drone de 20 mil?"). É **proibida** pergunta abstrata, retórica ou dirigida ao espectador ("você já parou pra pensar?"). O estranhamento é o gancho: o cérebro quer a resposta.
2. **CONTEXTUALIZAÇÃO**: o mínimo para a pergunta fazer sentido — e é aqui que assunto de nicho ganha a âncora pró-leigo ("o estreito por onde passa um quinto do petróleo do mundo", "a aliança militar que junta Estados Unidos e Europa").
3. **DESENVOLVIMENTO**: o miolo. O que aconteceu, com número, nome, **mecanismo** e a fonte nominal.
4. **CONSEQUÊNCIA**: uma só, concreta — o que muda no tabuleiro (quem ganha posição, que rota fecha, que preço sobe, o que este fato autoriza depois).
5. **CONCLUSÃO**: a **resposta** à pergunta da abertura, em uma frase seca que carrega a **disputa** do assunto — sobre o **fato** e sobre a **conta** (quem pagou, o que custou), nunca sobre quem é o vilão. Se a única forma de gerar disputa fosse acusar um lado, o roteiro fecha sem disputa. No Short ela emenda de volta na pergunta quando o vídeo reinicia (**loop**); no `--long-take` ela fecha de verdade, com o próximo marco a observar.
...
**Como a frase é escrita** (`INSTRUCOES_REDACAO`, 2026-08-15): o mesmo bloco entra nos **dois** formatos, porque o que muda entre o Short e o longo é a estrutura e a densidade, nunca o jeito de escrever a frase. São quatro regras que valem sobre qualquer preferência de estilo:

- **Factual** — cada frase carrega um fato do material recebido. Adjetivo que não é dado ("enorme", "histórico", "sem precedentes") sai e o **número** entra no lugar.
- **Simples** — a palavra comum sempre que ela diz a mesma coisa que a rara; uma ideia por frase, sujeito-verbo-objeto, **voz ativa**, sem oração encaixada. Simples não é bobo: quando a palavra comum **não** diz a mesma coisa, vale a palavra certa (a regra de VOCABULÁRIO manda nesse caso).
- **Assertiva** — sem hedge de estilo ("de certa forma", "pode-se dizer que", "aparentemente") e sem verbo de escapatória ("poderia vir a"). Presente ou passado simples.
- **Direta** — sem preâmbulo e sem meta-fala ("vamos entender", "neste vídeo"). A frase começa pela coisa concreta.

> **Isto não afrouxa a neutralidade, e no conflito a neutralidade vence.** Ser assertivo é cortar o hedge de **estilo**, nunca a **atribuição**: "segundo a Reuters", "o Ministério da Defesa russo diz" e "não confirmado" continuam obrigatórios onde a regra de neutralidade os exige. A diferença é que a frase atribuída também é dita de forma direta — "a Reuters confirmou o ataque com duas fontes", nunca "haveria indícios de que um ataque poderia ter ocorrido".
...
- **Não lê os últimos vídeos do canal (passo 3)** — tokens autorizados antes da ampliação de escopos só tinham `youtube.upload`. Rode `--auth-youtube` de novo para reautorizar com os escopos de leitura. Sem isso a execução aborta logo no início (a leitura alimenta a seleção guiada pela audiência).
- **Canal novo, sem vídeos publicados** — as primeiras execuções rodam com a régua de audiência **vazia** (`ultimos_publicados` e `top_retencao` não têm o que devolver). Isso é esperado e não aborta: a seleção decide pelo valor da informação e pelo material em vídeo até o canal acumular histórico. O rodízio de temas dos Shorts também só passa a morder depois do primeiro Short publicado.
- **Vídeo reprovado na auditoria por neutralidade** — a narração tomou partido, deu como fato a versão de um lado, ou citou número de baixas sem fonte. O log lista o trecho exato e o roteiro é reescrito uma vez. Se estiver reprovando muito, o material coletado provavelmente veio quase todo de contas de um lado só do conflito — o caminho é olhar `CONTAS_PADRAO`, não afrouxar a regra.
